...
" |>
  ...
  mutate(
    bits = str_extract(input, '^[0-9]+$'),
    command = str_extract(input, 'AND|OR|(L|R)SHIFT [0-9]+|NOT'),
    variables = str_extract_all(input, '[a-z]+')
         )
...
input_bits <-
  data |>
  filter(!is.na(bits)) |>
  transmute(name = output, value = bits) |>
  pivot_wider() |>
  as.list() |>
  map(as.numeric)
...
data |>
  ...
  mutate(
    fx =  case_when(
      ...
      command == 'OR' ~ 'bitwOr',
      command == 'NOT' ~ 'bitnot',
      command |> str_detect('LSHIFT') ~ 'bitwShiftL',
      command |> str_detect('RSHIFT') ~ 'bitwShiftR'
    ),
    fx = map(fx, rlang::as_function),
    shift = if_else(
      str_detect(command, 'SHIFT'),
      parse_number(input),
      NA_real_
    )
  )
